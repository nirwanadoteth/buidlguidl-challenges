...
contract DeployContracts is ScaffoldETHDeploy {
    function run() external ScaffoldEthDeployerRunner {
        ...
        console.logString(string.concat("Multisend Challenge deployed at: ", vm.toString(address(multisend))));
    }
    ...
    function deployMockTokens() internal returns (address, address) {
        ...
        console.logString(
            string.concat("Deployed mock tokens to:", vm.toString(address(scUSD)), " and ", vm.toString(address(scDAI)))
        );
