...
contract PredictionMarket is Ownable {
    ...
    function resolveMarketAndWithdraw() external onlyOwner predictionReported returns (uint256 ethRedeemed) {
        ...
        uint256 ethFromWinningTokens = 0;
        if (contractWinningTokens > 0) {
            ethFromWinningTokens = (contractWinningTokens * i_initialTokenValue) / PRECISION;
            // burn the winning tokens held by the contract
            s_winningToken.burn(address(this), contractWinningTokens);
            // reduce collateral by the redeemed amount
            s_ethCollateral -= ethFromWinningTokens;
        }
        uint256 totalEthToSend = ethFromWinningTokens + s_lpTradingRevenue;
        // reset LP trading revenue
    ...
    function sellTokensForEth(
        ...
        // Update LP trading revenue before sending ETH
        s_lpTradingRevenue -= ethToReceive;

        // Transfer tokens from user to contract
        bool ok1 = token.transferFrom(msg.sender, address(this), _tradingAmount);
        if (!ok1) {
            revert PredictionMarket__TokenTransferFailed();
        }
    ...
    function _calculatePriceInEth(
        ...
        // Liquidity checks
        if (!_isSelling && _tradingAmount > currentTokenReserve) {
            revert PredictionMarket__InsufficientLiquidity();
        }

        uint256 tokenTotalSupply = (_outcome == Outcome.YES) ? i_yesToken.totalSupply() : i_noToken.totalSupply();
        uint256 otherTokenTotalSupply = (_outcome == Outcome.YES) ? i_noToken.totalSupply() : i_yesToken.totalSupply();

        uint256 currentTokenSoldBefore = tokenTotalSupply - currentTokenReserve;
        uint256 currentOtherTokenSoldBefore = otherTokenTotalSupply - currentOtherTokenReserve;
        uint256 totalTokensSoldBefore = currentTokenSoldBefore + currentOtherTokenSoldBefore;

        // Avoid division by zero (shouldn't happen due to initial locking, but safe-guard)
        ...
        if (_isSelling) {
            // selling adds tokens back to reserve
            if (_tradingAmount > totalTokensSoldBefore) {
                revert PredictionMarket__InsufficientLiquidity();
            }
            currentTokenReserveAfter = currentTokenReserve + _tradingAmount;
        ...
        uint256 currentTokenSoldAfter = tokenTotalSupply - currentTokenReserveAfter;
